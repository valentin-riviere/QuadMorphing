...
// #define ROM_CALIBRATION // To use ROM calibration values
...
static uint8_t OSR = ADC_4096;   // set pressure amd temperature oversample rate
static uint8_t Gscale = GFS_2000DPS;	// SEEMS NOT WORKING WITH GUMSTIX (gres = 0 when gumstix start)
static uint8_t Ascale = AFS_2G;
static uint8_t Mscale = MFS_16BITS;// Choose either 14-bit or 16-bit magnetometer resolution
static uint8_t Mmode = 0x06;      // 2 for 8 Hz, 6 for 100 Hz continuous magnetometer data read
static float aRes, gRes, mRes;    // scale resolutions per LSB for the sensors
  
// Pin definitions

static uint16_t Pcal[8];       // calibration constants from MS5611 PROM registers
static unsigned char nCRC;     // calculated check sum to ensure PROM integrity
static uint32_t D1 = 0, D2 = 0;// raw MS5611 pressure and temperature data
...
static float magCalibration[3] = {0.9,1.17,0.96}, magbias[3] = {-350,368,-506};// Factory mag calibration and mag bias for raw data
static float gyroBias[3] = {0, 0, 0}, accelBias[3] = {ACC_BIAS_X, ACC_BIAS_Y, ACC_BIAS_Z};    // Bias corrections for gyro and accelerometer in robot frame
static int16_t tempCount;          // temperature raw count output
static float   temperature;        // Stores the MPU9250 gyro internal chip temperature in degrees Celsius
static double Temperature, Pressure;// stores MS5611 pressures sensor pressure and temperature
static float  SelfTest[6];         // holds results of gyro and accelerometer self test
static int16_t magSelfTest[3];     // self test values for magnetometer
...
void initSensors()
{
// Setup for Master mode, pins, external pullups, 400kHz for Teensy 3.1
  Wire.begin(I2C_MASTER, 0x00, I2C_PINS, I2C_PULLUP_EXT, I2C_RATE);
  I2Cscan();
...
// Self Test magnetometer
  AK8963SelfTest(magSelfTest);
  Serial.println("Check magnetometer self test value :");
  if(Mscale == MFS_14BITS) 
  {
		Serial.print("x-axis self test: -50 <= "); Serial.print(magSelfTest[0]); Serial.println(" <= 50");
	Serial.print("x-axis self test: -50 <= "); Serial.print(magSelfTest[1]); Serial.println(" <= 50");
	Serial.print("x-axis self test: -800 <= "); Serial.print(magSelfTest[2]); Serial.println(" <= -200");
  }
  else if(Mscale == MFS_16BITS)
  {
	Serial.print("x-axis self test: -200 <= "); Serial.print(magSelfTest[0]); Serial.println(" <= 200");
	Serial.print("x-axis self test: -200 <= "); Serial.print(magSelfTest[1]); Serial.println(" <= 200");
	Serial.print("x-axis self test: -3200 <= "); Serial.print(magSelfTest[2]); Serial.println(" <= -800");
  }
...
void IMU_getADC_full()
{
    ...
    Serial.print(" ay = "); Serial.print((int)1000*accADC[1]); 
    Serial.print(" az = "); Serial.print((int)1000*accADC[2]); Serial.println(" mg");
    Serial.print("gx = "); Serial.print( gyroADC[0] , 2); 
    Serial.print(" gy = "); Serial.print( gyroADC[1] , 2); 
    Serial.print(" gz = "); Serial.print( gyroADC[2] , 2); Serial.println(" rad/s");
    Serial.print("mx = "); Serial.print( (int)magADC[0] ); 
    Serial.print(" my = "); Serial.print( (int)magADC[1] ); 
    Serial.print(" mz = "); Serial.print( (int)magADC[2] ); Serial.println(" mG");
#endif
...
void calibrateAK8963(float * dest1, float * dest2) 
{
	...
	// Get hard iron correction
	 ...
	 Serial.println("Biases on raw data in Mag frame : "); Serial.println(dest1[0]); Serial.println(dest1[1]); Serial.println(dest1[2]); 
	...
	// Get soft iron correction estimate
	 ...
	 Serial.println("Mag calibration in Mag frame : "); Serial.println(dest2[0]); Serial.println(dest2[1]); Serial.println(dest2[2]); 

	 Serial.println("Mag Calibration done!");
}

void AK8963SelfTest(int16_t * destination)
{
	int16_t mData[3] = {0, 0, 0}, mAvg[3] = {0, 0, 0};

	writeByte(AK8963_ADDRESS, AK8963_CNTL, 0x00);	// Power down magnetometer
	delay(10);
	writeByte(AK8963_ADDRESS, AK8963_ASTC, 0x40);	// SELF = 1 of ASTC register
	writeByte(AK8963_ADDRESS, AK8963_CNTL, 0x08);	// Self_test mode
	delay(25);
	
	for( int ii = 0; ii < 200; ii++)	// Get average of 200 values
	{
		readMagData(&mData[0]);
		mAvg[0] += mData[0];
		mAvg[1] += mData[1];
	...
	destination[0] = mAvg[0];
	destination[1] = mAvg[1];
	destination[2] = mAvg[2];
	
	writeByte(AK8963_ADDRESS, AK8963_ASTC, 0x00);	// SELF = 0 of ASTC register 
	delay(10);
	writeByte(AK8963_ADDRESS, AK8963_CNTL, 0x00);	// Power down magnetometer  
	delay(10);
	writeByte(AK8963_ADDRESS, AK8963_CNTL, Mscale << 4 | Mmode);// Set magnetometer to normal operation
	delay(25);
}