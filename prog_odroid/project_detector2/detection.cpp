...
int detection(const Stream_in * p_s_in, Stream_out * p_s_out, bool * p_sh_start, sem_t * sem)
{
	...
	while (loop_main)
	{
		...
		float ttc = 0;
		uint64_t img_time_now, img_time_prev;
		...
		else
		{
			...
			// OpenCV var init
			Mat 	img_src = Mat(s_in_cpy.height,s_in_cpy.width,CV_8UC1),
					img_dst = Mat(s_in_cpy.height,s_in_cpy.width,CV_8UC1),
					img_now = Mat(H_SS, W_SS,CV_32F),
					img_prev = Mat(H_SS, W_SS,CV_32F);
...
t_start = std::chrono::high_resolution_clock::now();
	...
	cout << "Acquisition and copy shared vars (ms) : " << chrono::duration_cast<chrono::milliseconds>(std::chrono::high_resolution_clock::now() - t_start).count() << endl;
		...
		t_start = std::chrono::high_resolution_clock::now();
								...
								// Copy data for ttc computation
								img_now.copyTo(img_prev);
								img_time_prev = img_time_now;
			
								// Get acquisition time
								img_time_now = ptrGrabResult->GetTimeStamp();


	#ifdef BLUR
								// Reduce noise with blur
								blur(img_src, img_dst, Size(s_in_cpy.size_blur,s_in_cpy.size_blur));
								img_src = img_dst;
	#endif
		...
		t_start = std::chrono::high_resolution_clock::now();
								...
								SquaresDetector(img_src,squares,s_in_cpy.bin_square,s_in_cpy.k_square,s_in_cpy.area_square,s_in_cpy.cos_square);

								// Blur image and resize it
								blur_resize(img_src,img_now);
								GaussianBlur(img_now,img_now,Size(0,0),SIGMA_GAUSS); // Blur image

								// Compute Time-To-Collision (except for the 1st image)
								if (num_img > 0)
									ttc = compute_ttc(img_prev, img_now, (img_time_now-img_time_prev)*0.000000001, s_in_cpy.thresh_Et, s_in_cpy.n_median);
		...
		t_start = std::chrono::high_resolution_clock::now();
								...
								if (init_square_detection)
									cout << "Frame " << img_time_now << " : Subtented angles (in rad): " << sub_angles[0] << "\t" << sub_angles[1] << "\t" << sub_angles[2] << "\t" << sub_angles[3] << endl;
	#endif
	...
	sem_wait(sem);	// Critical Section
								...
								p_s_out->time_frame=img_time_now;
